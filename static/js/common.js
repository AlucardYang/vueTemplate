...
const csrfToken = document.querySelector('meta[name=csrf-token]').getAttribute('content'),
    ...
    fakeNum = 0,
    asyncScirpt = null,
    reg = [/^([2|3|5|7|8|6|9])\d{7}$/, /^([2|6])\d{7}$/, /^[1]\d{10}$/, /^\d{10}$/],
    mobile_pre = null,
    mobile = null,
...
$('#e-fav-precum').on('click', function () {
    if (isOnTheMini) {
        wx.miniProgram.navigateTo({
            url: '/pages/chat/chat?id=' + $(this).attr('data-id')
        })
    } else {
        if ($('#e-precum').length) {
...
$('.iber_link').on('click', function () {
    if (isOnTheMini) {
        ...
        wx.miniProgram.navigateTo({
            url: '/pages/broker/broker?id=' + $(this).attr('data-id')
        })
    } else {
...
Vue.prototype.isOnTheMini = isOnTheMini;
Vue.prototype.isWeiXin = isWeiXin;
Vue.prototype.showContact = showContact;
Vue.prototype.hideContact = hideContact;
Vue.prototype.newHtml = newHtml;